...
// Function: calculate_cnt2 is a modified version of calculate_cnt, which is used in the AlignFastLCS algorithm.
template <typename T>
void calculate_cnt2(std::vector<std::pair<double,int>> &angles, int N, int M, int &max_size_of_solution, double &max_theta){
    ...
    for (int a = 0; a < NM_angle_segments-1; a++){
        ...
        if (edge_num <= max_size_of_solution) continue;
...
// Function: A modified version of calculate_with_LCS_based_algorithm, which is used in the AlignFastLCS algorithm.
void calculate_with_LCS_based_algorithm2(std::vector<std::pair<double,int>> &angles, int N, int M, int &max_size_of_solution, double &max_theta) {
    ...
    if (M <= 64){
        ...
        if (M <= 192){
            calculate_cnt2<int192t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 256){
            calculate_cnt2<int256t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 320){
            calculate_cnt2<int320t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 384){
            calculate_cnt2<int384t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 448){
            calculate_cnt2<int448t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 512){
            calculate_cnt2<int512t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 576){
            calculate_cnt2<int576t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 640){
            calculate_cnt2<int640t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 704){
            calculate_cnt2<int704t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 768){
            calculate_cnt2<int768t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 832){
            calculate_cnt2<int832t>(angles, N, M, max_size_of_solution, max_theta);
        }else if (M <= 896){
            calculate_cnt2<int896t>(angles, N, M, max_size_of_solution, max_theta);
        }        
        else if (M <= 960){
            calculate_cnt2<int960t>(angles, N, M, max_size_of_solution, max_theta);
        }        
        else if (M <= 1024){
            calculate_cnt2<int1024t>(angles, N, M, max_size_of_solution, max_theta);
        }
        return;
    }else{
        std::cerr << "M is too large: " << M << std::endl;
...
// Function: compress is a helper function used to compress the sorted indices of points in P and Q, which is used in the ComputeMax2 function to reduce the size of the DP table.
std::vector<int> compress(std::vector<int>& vec) {
    std::vector<int> sorted_vec = vec;
    std::sort(sorted_vec.begin(), sorted_vec.end());

    sorted_vec.erase(std::unique(sorted_vec.begin(), sorted_vec.end()), sorted_vec.end());

    std::vector<int> compressed_vec;
    for (int& x : vec) {
        compressed_vec.push_back(std::lower_bound(sorted_vec.begin(), sorted_vec.end(), x) - sorted_vec.begin());
    }

    return compressed_vec;
}

// Function: ComputeMax2 is a modified version of ComputeMax, which is used in the AlignFastLCS algorithm. The main difference is that it uses compressed indices to reduce the size of the DP table, and it also takes the sorted indices of points in P and Q as input to facilitate the compression.
int ComputeMax2(const Eigen::Matrix3Xd& P, const Eigen::Matrix3Xd& Q, double threshold, Eigen::Matrix3Xd& res_R, std::string DP_type, std::vector<int> &sorted_indices_P, std::vector<int> &sorted_indices_Q) {

    int N = P.cols();
    int M = Q.cols();

    std::vector<int> sorted_indices_P_compressed = compress(sorted_indices_P);
    std::vector<int> sorted_indices_Q_compressed = compress(sorted_indices_Q);
    std::vector<std::pair<double, int>> angles;
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            ...
            int _i = sorted_indices_P_compressed[i];
            int _j = sorted_indices_Q_compressed[j];
            if (Pdist_from_Zaxis + Qdist_from_Zaxis < radius) {
                angles.push_back(std::make_pair(-M_PI, _i*M+_j+1));
                angles.push_back(std::make_pair(M_PI, -_i*M-_j-1));
            }else{
                ...
                double min_theta = theta - angle;
                double max_theta = theta + angle;
                push_back_angle_segment_with_index(angles, min_theta, max_theta, _i*M+_j+1);
    ...
    if (DP_type == "AlignFastLCS"){
        calculate_with_LCS_based_algorithm2(angles, N, M, max_size_of_solution, max_theta);
    }else{
        std::cerr << "Unknown DP type: " << DP_type << std::endl;
        exit(1);
    }
...
// Function: ALignFastPlus
// This function is almost the same as AlignFast, but it uses a pruning strategy to skip some iterations with the same theoretical guarantee.
int AlignFastPlus(std::string Algo, Eigen::Matrix3Xd& P, Eigen::Matrix3Xd& Q, double epsilon, double delta, Eigen::Matrix3Xd& res_P, Eigen::Matrix3Xd& res_Q) {
    ...
    for (int i = 0; i < N; i++){
        for (int j = 0; j < M; j++){
            ...
            // Sort the points in P by their distance from P.col(i) to facilitate pruning.
            std::vector<std::pair<double, int>> P_distances(N-i);
            for (int j = i; j < N; j++){
                P_distances[j-i] = std::make_pair(Distance(P.col(i), P.col(j)), j);
            }
            std::sort(P_distances.begin(), P_distances.end());
            std::vector<int> P_sorted_indices(N-i);
            for (int j = 0; j < N-i; j++){
                P_sorted_indices[j] = P_distances[j].second;
            }
            Eigen::Matrix3Xd P_sorted(3, N-i);
            for (int j = 0; j < N-i; j++){
                P_sorted.col(j) = P.col(P_sorted_indices[j]);
            }

            // Sort the points in Q by their distance from Q.col(j) to facilitate pruning.
            std::vector<std::pair<double, int>> Q_distances(M);
            for (int k = 0; k < M; k++){
                Q_distances[k] = std::make_pair(Distance(Q.col(j), Q.col(k)), k);
            }
            std::sort(Q_distances.begin(), Q_distances.end());
            std::vector<int> Q_sorted_indices(M);
            for (int k = 0; k < M; k++){
                Q_sorted_indices[k] = Q_distances[k].second;
            }
            Eigen::Matrix3Xd Q_sorted(3, M);
            for (int k = 0; k < M; k++){
                Q_sorted.col(k) = Q.col(Q_sorted_indices[k]);
            }

            for (int k = N-i-1; k > 0; k--){
                for (int l = M-1; l > 0; l--){

                    double Pi_Pk_distance = P_distances[k].first;
                    double Qj_Ql_distance = Q_distances[l].first;

                    if (abs(Pi_Pk_distance - Qj_Ql_distance) > 2.0*epsilon*(1.0+delta)) continue;

                    if (Pi_Pk_distance < 1e-6 || Qj_Ql_distance < 1e-6) continue;

                    if (P_sorted_indices[k] < i || Q_sorted_indices[l] < j) continue;

                    Eigen::Matrix4d transformP = computeTransformationMatrixToZero(P_sorted.col(0), P_sorted.col(k));
                    Eigen::Matrix3Xd P_tmp = transformP.block<3, 3>(0, 0) * P_sorted + transformP.block<3, 1>(0, 3).replicate(1, P_sorted.cols());

                    Eigen::Matrix4d transformQ = computeTransformationMatrixToZero(Q_sorted.col(0), Q_sorted.col(l));
                    Eigen::Matrix3Xd Q_tmp = transformQ.block<3, 3>(0, 0) * Q_sorted + transformQ.block<3, 1>(0, 3).replicate(1, Q_sorted.cols());

                    P_tmp.conservativeResize(Eigen::NoChange, k+1);

                    for (int idx = M-1; idx >= l; idx--){
                        if (Q_distances[idx].first - Pi_Pk_distance > 2.0*epsilon*(1.0+delta)){
                            Q_tmp.conservativeResize(Eigen::NoChange, Q_tmp.cols() - 1);
                        }else{
                            break;
                        }
                    }

                    std::vector<int> P_sorted_indices_tmp(k+1);
                    for (int idx = 0; idx <= k; idx++){
                        P_sorted_indices_tmp[idx] = P_sorted_indices[idx];
                    }
                    std::vector<int> Q_sorted_indices_tmp(Q_tmp.cols());
                    for (int idx = 0; idx < Q_tmp.cols(); idx++){
                        Q_sorted_indices_tmp[idx] = Q_sorted_indices[idx];
                    }
                                ...
                                for (int xj = -grid_num; xj <= grid_num; xj++){
                                    for (int yj = -grid_num; yj <= grid_num; yj++){
                                        for (int zj = -grid_num; zj <= grid_num; zj++){
                                            ...
                                            Eigen::Matrix4d transform = calculateTransformationMatrix(P_tmp.col(0), P_tmp.col(k), pi, pk);

                                            Eigen::Matrix3Xd P_transformed = transform.block<3, 3>(0, 0) * P_tmp + transform.block<3, 1>(0, 3).replicate(1, P_tmp.cols());

                                            Eigen::Matrix3Xd R_tmp;

                                            // The following function, ComputeMax, is implemented such that the l-axis is aligned with the z-axis to simplify the implementation.
                                            int size_of_solution = ComputeMax2(P_transformed, Q_tmp, (1.0+delta)*epsilon, R_tmp, Algo, P_sorted_indices_tmp, Q_sorted_indices_tmp);
                                            if (size_of_solution > max_size_of_solution){
                                                max_size_of_solution = size_of_solution;
                                                res_P = transformP.block<3, 3>(0, 0) * P + transformP.block<3, 1>(0, 3).replicate(1, N);
                                                res_P = transform.block<3, 3>(0, 0) * res_P + transform.block<3, 1>(0, 3).replicate(1, N);
                                                res_Q = transformQ.block<3, 3>(0, 0) * Q + transformQ.block<3, 1>(0, 3).replicate(1, M);
                                                res_Q = R_tmp * res_Q;
                                                R = R_tmp;
                                            }
                                        }
...
int solve_with_AlignFastPlus(Eigen::Matrix3Xd& P, Eigen::Matrix3Xd& Q, double epsilon, double delta, Eigen::Matrix3Xd& res_P, Eigen::Matrix3Xd& res_Q) {
    return AlignFastPlus("AlignFastLCS", P, Q, epsilon/(1.00000+delta), delta, res_P, res_Q);
}
...
int main(int argc, char* argv[]) {
    ...
    program.add_argument("--Algorithm")
        ...
        .action([](const std::string& value) {
            if (value == "LiNgAlign" || value == "BasicAlign" || value == "AlignFastLIS" || value == "AlignFastLCS" || value == "AlignFastPlus") {
                return value;
            } else {
                throw std::runtime_error("Invalid algorithm specified. Use 'LiNgAlign' or 'BasicAlign' or 'AlignFastLIS' or 'AlignFastLCS' or 'AlignFastPlus'.");
            }
        });
    ...
    }else if (program.get<std::string>("--Algorithm") == "AlignFastPlus") {
        std::cerr << "Using AlignFastPlus algorithm." << std::endl;
        int solution_size = solve_with_AlignFastPlus(P, Q, epsilon, delta, res_P, res_Q);
        std::cout << "Solution size with AlignFastPlus: " << solution_size << std::endl;
        std::cerr << "AlignFastPlus completed." << std::endl;
    }else{
